...
class Api(object):
  ...
  def shorten(self,longURLs,params={}):
    ...
    self._CheckForError(json_data)

    results = json_data['results']
    res = [self._extract_short_url(results[url]) for url in longURLs if url in results]
  ...
  def expand(self,shortURL,params={}):
    ...
    json_data = json.loads(result.decode(), 'utf-8')
    self._CheckForError(json_data)
    return json_data['results'][shortURL.split('/')[-1]]['longUrl']
  ...
  def info(self,shortURL,params={}):
    ...
    json_data = json.loads(result.decode(), 'utf-8')
    self._CheckForError(json_data)
    return json_data['results'][shortURL.split('/')[-1]]
  ...
  def stats(self,shortURL,params={}):
    ...
    json_data = json.loads(result.decode(), 'utf-8')
    self._CheckForError(json_data)
    return Stats.NewFromJsonDict(json_data['results'])
